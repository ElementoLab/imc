...
from imc.utils import read_image_from_file, estimate_noise, double_z_score, cleanup_channel_names
...
def parse_acquisition_metadata(
    ...
    if acquisition_id is not None:
        acquired = acquired.loc[acquired['AcquisitionID'].isin([acquisition_id, str(acquisition_id)])]
    ...
    assert all(reference.query("ilastik == True").index == joint_panel.query("ilastik == True").index)
...
def check_channel_axis_correlation(
        ...
        output_prefix: Path) -> DataFrame:
    # # Plot and regress
    __n, __m = get_grid_dims(arr.shape[0])
    fig, axis = plt.subplots(__m, __n, figsize=(__n * 4, __m * 4), squeeze=False, sharex=True, sharey=True)
    ...
    axis[int(__m / 2), 0].set_ylabel("Mean signal along axis")
    axis[-1, int(__n / 2)].set_xlabel("Order along axis")
    c = sns.color_palette("colorblind")
    patches = [mpatches.Patch(color=c[0], label="X"), mpatches.Patch(color=c[1], label="Y")]
    axis[int(__m / 2), -1].legend(
...
def fix_signal_axis_dependency(
    ...
    for channel in range(arr.shape[0]):
        ...
        for ax in axis[:-2]:
            ax.axis('off')
        fig.savefig(
            output_prefix + f"channel-axis_correlation_removal.{channel_labels[channel]}.demonstration.svg",
            **FIG_KWS)
        plt.close("all")
    return corr_d
...
def measure_channel_background(
    ...
    noise = pd.Series(
        np.fromiter(map(
            estimate_noise, [lay for roi in rois for lay in roi.stack]
        ), float).reshape(len(rois), -1).mean(0),
        index=cmeans.index)
    ...
    sigmas = pd.DataFrame(
        ...
        columns=rois[0].channel_labels).T

    # Join all metrics
    metrics = (
        cmeans.mean(1).to_frame(name="cell_mean")
...
def single_cell_analysis(
    ...
    sc.pl.heatmap(
        ann, ann.var.index,
        log=True, standard_scale="obs", use_raw=False, show=False, groupby='roi')
    ...
    sc_kwargs = dict(
        color=variables,
        show=False, return_fig=True, use_raw=True)
    ...
    fig.savefig(
        output_prefix + 'cell_count_per_cluster.barplot.svg',
        **FIG_KWS)

    # Plot abundance per cluster
    cluster_counts_per_roi = (
        ann.obs.groupby(['cluster'] + cats)
        .count().iloc[:, 0].rename("ROI").reset_index()
    ...
    for ax, log in zip(axes, [False, True]):
        sns.heatmap(
            ...
            robust=True, ax=ax, square=True)
    sns.heatmap(
        (cluster_counts_per_roi / cluster_counts_per_roi.sum()) * 100,
        cbar_kws=dict(label="Cells per cluster (%)"),
        robust=True, ax=axes[2], square=True)
    fig.savefig(
        output_prefix + 'cell.counts_per_cluster_per_roi.svg',
        **FIG_KWS)
    ...
    kwargs = dict(
        row_colors=row_means, row_colors_cmaps=["Greens"],
        col_colors=col_counts, col_colors_cmaps=["Purples"],
        metric='correlation', robust=True,
    ...
    for label1, df in [
        ...
        ("filtered_clusters", mean_expr.loc[
    ...
    sc_kwargs = dict(
        color=variables + ann_ct.var.index.tolist(),
        show=False, return_fig=True, use_raw=True)
    fig = sc.pl.pca(ann, **sc_kwargs)
    rasterize_scanpy(fig)
...
def derive_reference_cell_type_labels(
    ...
    assign = {
        ch: OrderedSet(
            propert
            for marker, propert in act_labels.items()
            if marker in label)
        for ch, label in labels.items()}
    new_labels = pd.Series({
        k: ", ".join(v)
        for k, v in assign.items()
    }).sort_index().rename("cell_type").rename_axis("cluster")
    ...
    __t = mean_expr_z >= _v1
    kwargs = dict(
        metric='correlation', robust=True, xticklabels=True, yticklabels=True,
    ...
    kwargs = dict(
        metric='correlation', robust=True, xticklabels=True, yticklabels=True,
        center=0, cmap="RdBu_r", cbar_kws=dict(label="Mean expression (Z-score)"),
        row_colors=cmeans, row_colors_cmaps=['Greens'],
        col_colors=fractions, col_colors_cmaps=["Purples"])
    opts = [
    ...
    grid = sns.clustermap(
        mean_expr_z_l.corr(), center=0, cmap="RdBu_r",
        cbar_kws=dict(label="Pearson correlation"),
        metric='correlation', robust=True,
        xticklabels=True, yticklabels=True,
        row_colors=fractions_l, row_colors_cmaps=['Purples'],
        col_colors=fractions_l, col_colors_cmaps=['Purples'])
    grid.savefig(
...
def predict_cell_types_from_reference(
    ...
    os.makedirs(os.path.dirname(output_prefix), exist_ok=True)
    default_ref = (
        sample.prj.processed_dir / 'single_cell' /
    ...
    grid = sns.clustermap(
        corrwithref, center=0, cmap="RdBu_r",
    ...
    grid = sns.clustermap(
        query_means
        .rename(columns=pred_cell_type_labels)
        .rename_axis("Predicted cell types"),
...
def get_adjacency_graph(
    ...
    lc = graph.show_rag(
        mask, g, (image_mean * 255).astype(int),
        ax=ax, img_cmap="viridis", edge_cmap="Reds", edge_width=1)
    ax.axis('off')
    ...
    nx.set_node_attributes(
        g, roi.clusters.astype(str).to_dict(),
        name="cluster")
    # save graph
...
def measure_cell_type_adjacency(
        ...
        output_prefix: Optional[Path] = None) -> DataFrame:
    # import community
    ...
    shuffled_freq.to_csv(
        output_prefix + f"cluster_adjacency_graph.random_frequencies.all_iterations_{n_iterations}.csv")
...
def find_communities(
        ...
        plot: bool = True) -> Tuple[Series, Tuple]:
    ...
    def get_community_cell_type_composition(roi: "ROI", partition: Series):
        ...
        for com, members in get_community_members(partition).items():
            ...
            cts[com] = \
                roi.clusters.loc[members].value_counts()
        return (
            pd.DataFrame(cts)
            .fillna(0)
    ...
    partition = pd.Series(
        community.best_partition(
            roi.adjacency_graph, resolution=community_resolution),  # , weight="expr_weight")
        name="community").sort_index()
    n = partition.value_counts().shape[0]
    tqdm.write(f"Found {n} communities for ROI {roi}.")
    partition += 1
    partition.to_csv(
        roi_output_prefix + "graph_partition.csv")
    comps = (
    ...
    if plot:
        ...
        grid = sns.clustermap(
            percent,
            metric="correlation", cbar_kws=dict(label="% of cell type"))
        grid.savefig(roi_output_prefix + "cell_type_composition.svg", **FIG_KWS)
        grid = sns.clustermap(
            percent, z_score=1, cmap="RdBu_r", center=0,
            metric="correlation", cbar_kws=dict(label="% of cell type (Z-score)"))
        grid.savefig(
...
def cluster_communities(
    ...
    sc.pp.neighbors(a)
    sc.tl.leiden(
        a, resolution=supercommunity_resolution,
        key_added="supercommunity")
    ...
    for roi in rois:
        ...
        if roi.clusters.dtype == "int" and roi.clusters.min() == 0:
            c1 = roi.clusters + 1  # TODO: this +1 should be removed when clustering is re-run since the new implm
        else:
            c1 = roi.clusters
        # {cell: community}
        c2 = (
            pd.Series(partitions[roi], name="community")
    ...
    comp = assignments.assign(count=1).pivot_table(
        index='supercommunity', columns='cluster',
        values='count', aggfunc=sum, fill_value=0)

    perc = (comp.T / comp.sum(1)).T * 100
    diffs = pd.DataFrame(
        np.sqrt(abs(perc.values - perc.values[:, None]).sum(axis=2)),
        index=perc.index, columns=perc.index)
    grid = sns.clustermap(diffs)
    repl = pd.Series(dict(zip(
        grid.data.columns,
        fcluster(
            grid.dendrogram_col.linkage, t=max_supercommunity_difference,
            criterion='distance')))).sort_index()
    ...
    grid = sns.clustermap(
        ...
        row_linkage=grid.dendrogram_row.linkage, col_linkage=grid.dendrogram_col.linkage,
        metric="correlation",
        row_colors=plt.get_cmap("tab20")(a.obs['supercommunity'].astype(int)),
        rasterized=True)
    grid.savefig(
        output_prefix + "communities.cell_type_composition.leiden_clustering.clustermap_viz.counts.svg",
        **FIG_KWS)
    for method in ['pca', 'umap']:
        fig = getattr(sc.pl, method)(
            a, color=['supercommunity'] + a.var.index.tolist(),
    ...
    grid = sns.clustermap(
        diffs, col_colors=plt.get_cmap("tab20")(repl.values),
    ...
    for var_ in ['community', 'supercommunity']:
        supercts = (
            assignments.assign(count=1)
            .pivot_table(
                index='cluster', columns=var_,
                values='count', aggfunc=sum, fill_value=0))
        ...
        grid = sns.clustermap(
            perc_supercts, z_score=1, cmap="RdBu_r", center=0,
            metric="correlation", rasterized=True,
    ...
    fig, axes = plt.subplots(
        __n, __m, figsize=(4 * __m, 4 * __n), squeeze=False, sharex='row', sharey='row')
